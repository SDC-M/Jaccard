...
fichiers passés sur la ligne de commande. Cette fonction est divisé en deux parties.

\begin{lstlisting}[style=Cstyle]
  static int scptr_display(context *ctx, const char *ref); 
\end{lstlisting}
Cette fonction permet d'afficher une ligne du graph, pour se faire le procésus est assez simple,
...
à chauqe mot de l'arbre des unions.

\paragraph{L'option -iVALUE / --initial=VALUE:}
Cette option permet de fixer la taille des mots à \textbf{VALUE}.
Celle-ci est traité au moment de la lecture des mots et permet de signaler qu'il faut ignorer tous les caractères dépassant cette
limite jusqu'au prochain caractère de séparation (voir \textbf{\ref{subsec:fscanf}}).
...
doivent être concidérer comme des séparateurs pour segmentation des mots.
...
s'offraient alors à nous, créer l'arbre depuis l'entrée standart directement ou bien de stocker l'entrée standart dans un fichier
puis effectuer le même traitement que sur les autres arguments sans distinction aucune. Dans un soucis d'efficacité
c'est donc la première solution qui a été choisie. Dans le cas où l'on détécte '-' à la place d'un nom de fichier alors nous avons plus
qu'à lire depuis l'entrée standart.
...
Nous avons égelment décider de tester cette solution en parallèle et nous en faisons référence dans
la section \textbf{\ref{sec:perf}} liée aux performances de l'implémentation proposée.
Deux versions de ce module ont été réfléchis pendant la création de ce projet. La premiere en utilisant le 
...
Nous avons finalement décider de ne pas utiliser ce module pour avoir une performance optimale sur notre exécutable.
Néamoins nous avons réaliser une ptite étude sur une petit jeu de données afin de se rendre compte des différences.
...
\begin{tikzpicture}
  ...
  \begin{tabular}{|>{\raggedright\arraybackslash}m{4.5cm}|>{\centering\arraybackslash}m{2.2cm}|>{\centering\arraybackslash}m{2.3cm}|>{\centering\arraybackslash}m{2.2cm}|>{\centering\arraybackslash}m{2.8cm}|>{\centering\arraybackslash}m{2.5cm}|}
  \hline
  \rowcolor{gray!20}
  ...
  \hline
  abeeeillmrsss / lesmiserables.txt & 3.1 / 3.1 & 60242 / 60242 & 2.557 & 13062070 & 0 \\
  \hline
  ...
  \hline
  fr\_.txt / domjuan.txt & 3.9 / 0.1007 & 336528 / 4255 & 1.165 & 23098730 & 0.9957 \\
  \hline
  \end{tabular}
  };
\end{tikzpicture}
...
\begin{tikzpicture}
  ...
  \textbf{Fichiers} & \textbf{Tailles (MB)} & \textbf{Nombre de mots} & \textbf{Temps réel (s)} & \textbf{Allocations (bytes)} & \textbf{Distance de Jaccard} \\
  \hline
  abeeeillmrsss / lesmiserables.txt & 3.1 / 3.1 & 60242 / 60242 & 38.538 & 12707198 & 0 \\
  \hline
  ...
  \hline
  fr\_.txt / domjuan.txt & 3.9 / 0.1007 & 336528 / 4255 & 96.309 & 25510530 & 0.9957 \\
  \hline
  \end{tabular}
  };
\end{tikzpicture}
...
\begin{tikzpicture}
  ...
  \textbf{Fichiers} & \textbf{Tailles (MB)} & \textbf{Nombre de mots} & \textbf{Temps réel (s)} & \textbf{Allocations (bytes)} & \textbf{Distance de Jaccard} \\
  \hline
  abeeeillmrsss / lesmiserables.txt & 3.1 / 3.1 & 60242 / 60242 & 0.848 & 11134310 & 0 \\
  \hline
  ...
  \hline
  fr\_.txt / domjuan.txt & 3.9 / 0.1007 & 336528 / 4255 & 0.393 & 17646186 & 0.9957 \\
  \hline
  \end{tabular}
  };
\end{tikzpicture}
...
\paragraph{Avec LDSC:} Comme nous pouvons le remarquer dans le premier tableau, les statistique temporelle sont relativement satisfaisantes 
lorsque nous les comparons avec celles concernant une implémentation par tableaux dynamiques. Ce qui pose problème ici c'est le nombres d'allocation.
En effet ce nombres est le plus élevé des trois implémentations ci-dessus. Ce qui est cohérent puisque nous allouons un cellules de liste qui contient
\texttt{la référence} et un champs \texttt{next} pour chaque mots trouvé dans les fichiers.
Nous pouvons donc l'améliorer en essayant de réduire le nombre d'allocations.

\paragraph{Avec tableau dynamique:} Ce qui, à première vue, semblait une améliorations possibles de notre projets, c'est avérée la pire implémentation de
celles proposées dans notre projets. Nous avions effectivement réduit le nombre d'allocations par rapport au LDSC, mais nous perdions un temps considérable
...
\textbf{\texttt{abeeeillmrsss.txt}} et \textbf{\texttt{lesmiserables.txt}}. Nous ne pouvions pas rester sur cette implémentations.

\paragraph{Sans module \texttt{Holdall:}} En effet supprimer totalement ce module permettrais de supprimer beacoup d'allocations et donc de gagner du temps.
De plus nous stockons déjà toutes les références des mots dans les "AVL fichiers", nous avons donc utiliser la fonction \textbf{bst\_dft\_infix\_apply\_context}
pour nous permettres de parcourir tous les AVL et ainsi libérer les références. Et cette méthode c'est avérée la meilleure au vu des résultats. Avec les mêmes 
textes cités précédemment nous avons pu divisé le temps par 45 par rapport à l'implémentation par tableau dynamique, et environ 2M d'octets de gagnés 
par rapport au LDSC.

\section{Améliorations possibles}

\paragraph{Une implémentation totalement différentes:} Nous avons remarqué grâce à l'executable fourni sur universitice 
...
tous les mots distincts en temps que clés et l'appartenance dans les les fichiers en tant que valeurs, nous aurions eu besoin d'allouer 
seulement la table de hachages; là où, pour notre projet, nous avions besoin d'un AVL par fichiers et également un AVL d'union totale 
pour l'affichage du graphe lorsque celui-ci est demandé. Nous perdons beaucoup de temps à allouer les ressources nécessaire. 

\paragraph{N'allouer qu'une seule fois les références:} Un autre problème de notre implémentation c'est vu qu'on créer un AVL par fichier
ce qui cause plusieurs allocation d'un même mots lorsque celui-ci existe dans plusieurs fichiers. La solution serait de stocker des pointeurs
...
les restrictions donné dans le sujet, c'est-à-dire: 
\begin{center}
