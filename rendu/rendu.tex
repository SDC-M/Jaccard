...
avons pu communiquer efficacement sur les tâches que nous réalisions. 
...
Les paragraphes qui suivent préciseront ce que ces options font et comment elles le font. Il est a noté que nous ne supportons pas
...
Cette option permet d'indiquer à la fonction de lecture que les caractères de ponctuation de la classe \texttt{ispunct}
doivent être considérés comme des séparateurs pour la segmentation des mots.
...
Cela se fait tout simplement par le fait d'effectuer le test de comparaison de la présence de l'option en premier afin de stocker le nom du fichier.
...
\paragraph{Toute autre option renvoie un message d'erreur associé et met fin à l'éxécution du programme.}
...
une petite astuce de la théorie des ensembles qui nous dit que : Le cardinal de l'union de deux ensemble n'est rien d'autre
...
\textbf{Avec module holdall avec des tableaux dynamique sur la version finale du projet}
\begin{center}
\begin{tikzpicture}
  ...
  \hline
  abeeeillmrsss / lesmiserables.txt & 3.1 / 3.1 & 60242 / 60242 & \textbf{1.898} & \textbf{8876271} & 0 \\
  \hline
  tartuffe.txt / lesmiserables.txt & 0.1163 / 3.1 & 4804 / 60242 & \textbf{1.071} & \textbf{6735085} & 0.9474 \\
  \hline
  tartuffe.txt / domjuan.txt & 0.1163 / 0.1007 & 4804 / 4255 & \textbf{0.072} & \textbf{872443} & 0.7852 \\
  \hline
  fr\_.txt / domjuan.txt & 3.9 / 0.1007 & 336528 / 4255 & \textbf{0.940} & \textbf{39530237} & 0.9957 \\
  \hline
  \end{tabular}
  };
\end{tikzpicture}
...
\textbf{Avec module holdall avec LDSC sur la version finale du projet}
\begin{center}
\begin{tikzpicture}
  \node[inner sep=0pt] (table) {
    \begin{tabular}{|>{\raggedright\arraybackslash}m{3.3cm}|
  ...
  \textbf{Fichiers} & \textbf{Tailles (MB)} & \textbf{Nombre de mots} & \textbf{Temps réel (s)} & \textbf{Allocations (bytes)} & \textbf{Distance de Jaccard} \\
  \hline
  abeeeillmrsss / lesmiserables.txt & 3.1 / 3.1 & 60242 / 60242 & \textbf{1.903} & \textbf{8795655} & 0 \\
  \hline
  tartuffe.txt / lesmiserables.txt & 0.1163 / 3.1 & 4804 / 60242 & \textbf{1.076} & \textbf{6679317} & 0.9474 \\
  \hline
  tartuffe.txt / domjuan.txt & 0.1163 / 0.1007 & 4804 / 4255 & \textbf{0.075} & \textbf{864771} & 0.7852 \\
  \hline
  fr\_.txt / domjuan.txt & 3.9 / 0.1007 & 336528 / 4255 & \textbf{0.935} & \textbf{36574677} & 0.9957 \\
  \hline
  \end{tabular}
  };
\end{tikzpicture}
...
\paragraph{L'appartenance des mots :}
Nous avons dans notre implémentation un arbre par fichier cependant ces arbres ne servent en réalité qu'à définir l'appartenance des mots
ce qui est en réalité très couteux, la solution pourrait-être de revoir la structure de tout le projet et de n'avoir que l'arbre des unions
ne contenant cette fois non pas un pointeur vers le mot mais une structure avec un témoin d'appartenance. Nous avions vu en cours le moyen de
regarder sur un octet si a la k-ième place se trouvait un "1" nous pourrions donc théoriquement le mettre en pratique ici. Cela nous imposerait une
limite de fichiers bien que nous pourrions avoir plusieurs size\_t pour cela. Dans le cas ou nous en utiliserions qu'un seul notre éxécutable ne pourrait
alors supporter alors que 64 fichiers comme l'éxécutable d'exemple fournit. Par manque de temps nous n'avons pas pu revoir tout notre code.
\end{document}
